...
//		analyzeAndClean("C:\\Users\\Jack\\Desktop\\Cullen\\tempDataDump\\",2);
		//look("C:\\Users\\Jack\\Desktop\\Cullen\\tempDataDump\\shapePatterns2");
			...
			for(int i = 0; i < tree.root.underlings.size();i++){
				for(int j = i+1; j < tree.root.underlings.size();j++){
					if(isSame(tree.root.underlings.get(i).map, tree.root.underlings.get(j).map)){
						//Parcel.trace("Dupe");
						tree.root.underlings.remove(j);
